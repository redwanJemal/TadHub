                ...
                <div className="border rounded-md max-h-40 overflow-y-auto">
                  {contractsData.items.map((c) => (
                    <button key={c.id} type="button" className="w-full text-start px-3 py-2 hover:bg-muted text-sm"
                      onClick={() => { setContractId(c.id); setContractSearch(`${c.contractCode}${c.worker?.fullNameEn ? ` - ${c.worker.fullNameEn}` : ''}`); }}>
                      <span className="font-medium font-mono">{c.contractCode}</span>
                      {c.worker?.fullNameEn && (
                        <span className="text-muted-foreground ms-2 text-xs">{c.worker.fullNameEn}</span>
                      )}
                    </button>
                  ))}
                </div>
                ...
                <div className="border rounded-md max-h-40 overflow-y-auto">
                  {clientsData.items.map((c) => (
                    <button key={c.id} type="button" className="w-full text-start px-3 py-2 hover:bg-muted text-sm"
                      ...
                      {c.nameAr && (
                        <span className="text-muted-foreground ms-2 text-xs">{c.nameAr}</span>
                      )}
                    </button>
                  ))}
                </div>
                ...
                <div className="border rounded-md max-h-40 overflow-y-auto">
                  {suppliersData.items.map((s) => (
                    <button key={s.id} type="button" className="w-full text-start px-3 py-2 hover:bg-muted text-sm"
                      onClick={() => { setSupplierId(s.id); setSupplierSearch(`${s.supplier?.nameEn ?? ''}${s.supplier?.nameAr ? ` (${s.supplier.nameAr})` : ''}`); }}>
                      <span className="font-medium">{s.supplier?.nameEn ?? '—'}</span>
                      {s.supplier?.nameAr && (
                        <span className="text-muted-foreground ms-2 text-xs">{s.supplier.nameAr}</span>
                      )}
                    </button>
                  ))}
                </div>
